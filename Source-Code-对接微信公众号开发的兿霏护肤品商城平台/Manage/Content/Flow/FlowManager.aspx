       ...
       <asp:TemplateField HeaderText="流程名称" ItemStyle-CssClass="text-left">
		...
		<asp:TemplateField HeaderText="流程操作" ShowHeader="False" ItemStyle-CssClass="td_l">
			<ItemTemplate>
				...
				<a href='FlowProcess.aspx?id=<%# Eval("id") %>&name=<%# Eval("flowName") %>' class="option_style"><i class="fa fa-plus" title="添加"></i>步骤</a>
                <a href='FlowProcessManager.aspx?id=<%# Eval("id") %>&name=<%#Eval("flowName") %>' class="option_style"><i class="fa fa-list-alt" title="列表"></i>列表</a>
			</ItemTemplate>
		</asp:TemplateField>
	</Columns>
</ZL:ExGridView>
